...
def get_all_uncorrupted_profiles_and_indices(data_storage_filename, metadata_dict):
    indices = get_all_uncorrupted_indices(metadata_dict)
    sorted_indices = sort_indices_by_run_number_and_profile_number(indices)
    return get_beam_profiles_from_indices(data_storage_filename, sorted_indices), np.array(indices)
